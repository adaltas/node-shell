    ...
    this.register({
      configure_set: function({config, command}, handler) {
        ...
        if (!(config.router.stdout instanceof stream.Writable)) {
          throw error(["Invalid Configuration Property:", "router.stdout must be an instance of stream.Writer,", `got ${JSON.stringify(config.router.stdout)}`]);
        }
        if (!(config.router.stderr instanceof stream.Writable)) {
          throw error(["Invalid Configuration Property:", "router.stderr must be an instance of stream.Writer,", `got ${JSON.stringify(config.router.stderr)}`]);
        }
        return handler;
      }
    });
    ...
    this.register({
      configure_set: function({config, command}, handler) {
        ...
        if ((ref = typeof config.handler) !== 'function' && ref !== 'string') {
          throw error(['Invalid Route Configuration:', "accept string or function", !command.length ? "in application," : void 0, command.length ? `in command ${JSON.stringify(command.join(' '))},` : void 0, `got ${JSON.stringify(config.handler)}`]);
        }
        return handler;
      }
    });
