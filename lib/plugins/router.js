...
Parameters.prototype.route = function(argv = process, ...args) {
  var appconfig, command, config, err, i, index, params, route, route_call, route_error, route_from_config;
  // Normalize arguments
  ...
  route_from_config = (config, command, params) => {
    ...
    if (!route) {
      ...
      if (!(Object.keys(config.commands).length || route)) {
        err = config.root ? error(['Missing Application Route:', 'a \"route\" definition is required when no command is defined']) : error(['Missing Command Route:', `a "route" definition ${JSON.stringify(params[appconfig.command])} is required when no child command is defined`]);
      }
