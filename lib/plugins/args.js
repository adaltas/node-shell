...
// Dependencies
var Parameters, clone, is_object_literal, merge, set_default, utils,
  indexOf = [].indexOf;
    ...
    for (_ in ref2) {
      ...
      if (option.required) {
        if (params[option.name] == null) {
          throw utils.error(['Required Option Argument:', `the \"${option.name}\" option must be provided`]);
        }
      }
      if (option.one_of) {
        ...
        if (!option.required && values !== void 0) {
          ...
          for (i = 0, len = values.length; i < len; i++) {
            ...
            if (indexOf.call(option.one_of, value) < 0) {
              throw utils.error(['Invalid Argument Value:', `the value of option \"${option.name}\"`, `must be one of ${JSON.stringify(option.one_of)},`, `got ${JSON.stringify(value)}`]);
            }
          }
        }
      }
  ...
  compile = function(config, lparams) {
    ...
    for (_ in ref) {
      ...
      if (option.required && (value == null)) {
        ...
        throw utils.error(['Required Option Parameter:', `the \"${key}\" option must be provided`]);
      }
      ...
      if ((value != null) && option.one_of) {
        ...
        for (i = 0, len = value.length; i < len; i++) {
          ...
          if (indexOf.call(option.one_of, val) < 0) {
            throw utils.error(['Invalid Parameter Value:', `the value of option \"${option.name}\"`, `must be one of ${JSON.stringify(option.one_of)},`, `got ${JSON.stringify(val)}`]);
          }
        }
      }
      // Serialize
