...
Parameters = function(config = {}) {
  var sanitize_command, sanitize_commands, sanitize_commands_enrich, sanitize_help_command, sanitize_options, sanitize_options_enrich;
  this.config = config;
  // Sanitize options
  ...
  sanitize_command = function(command, parent) {
    ...
    if (command.command != null) {
      ...
      throw Error(`Invalid Configuration: command property can only be declared at the application level, not inside a command, got ${command.command}`);
    }
...
Parameters.prototype.run = function(argv = process, ...args) {
  var commands, extended, helpconfig, params, run;
...
Parameters.prototype.stringify = function(params, options = {}) {
  var argv, extended, keys, stringify;
  argv = options.script ? [process.execPath, options.script] : [];
  extended = options.extended || this.config.extended;
  ...
  stringify = (config, lparams) => {
    ...
    // Recursive
    has_child_commands = extended ? params.length : Object.keys(config.commands).length;
    if (has_child_commands) {
      command = extended ? params[0][this.config.command] : params[this.config.command].shift();
      if (!config.commands[command]) {
      ...
      stringify(config.commands[command], extended ? params.shift() : lparams);
    }
    if (extended || !has_child_commands) {
...
// Note, they are always pushed to the end and associated with the deepest child
  ...
  stringify(this.config, extended ? params.shift() : params);
  return argv;
};
...
Parameters.prototype.helping = function() {
  var args, commands, helping, leftover, params, search;
...
Parameters.prototype.help = function() {
  var _, arg, args, cmd, command, commands, config, configs, content, description, has_help_command, has_help_option, i, j, l, len, len1, len2, len3, len4, line, m, n, name, o, option, options, ref, ref1, ref2, ref3, ref4, ref5, shortcut, synopsis;
  args = Array.prototype.slice.call(arguments);
  // Get options
  if (args.length > 1) {
  ...
  if (is_object(args[0])) {
    if (args.length > 1) {
      throw Error('Invalid Arguments: only one argument is expected if first argument is an object');
    }
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (typeof arg !== 'string') {
        throw Error(`Invalid Arguments: argument is not a string, got ${JSON.stringify(arg)}`);
      }
    }
    commands = args[0];
  ...
  config = this.config;
  configs = [config];
  for (i = m = 0, len2 = commands.length; m < len2; i = ++m) {
    command = commands[i];
  ...
  synopsis = [];
  for (i = n = 0, len3 = configs.length; n < len3; i = ++n) {
    config = configs[i];
