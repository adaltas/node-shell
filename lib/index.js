...
Parameters.prototype.configure = function(config) {
  ...
  sanitize_route = function(config) {
    ...
    if ((ref = typeof config.route) !== 'function' && ref !== 'string') {
      throw Error(['Invalid Route Configuration: ', "accept string or function ", !Array.isArray(config.command) ? "in application, " : void 0, Array.isArray(config.command) ? `in command ${JSON.stringify(config.command.join(' '))}, ` : void 0, `got ${JSON.stringify(config.route)}`].join(''));
    }
  };
  ...
  route = (config, commands) => {
    ...
    if (!route) {
      ...
      if (!(Object.keys(config.commands).length || route)) {
        error = config.root ? Error("Missing \"route\" definition for application") : Error(`Missing "route" definition for command ${JSON.stringify(params[this.config.command])}`);
      }
...
Parameters.prototype.parse = function(argv = process, options = {}) {
  ...
  parse = (config, command) => {
    var _, child_params, error, helping, j, key, leftover, len, main, option, params, ref, ref1, ref2, shortcut, type, value, values;
