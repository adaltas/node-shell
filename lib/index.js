 ...
 */
var Parameters, array_to_object, is_object, load, pad, types,
  indexOf = [].indexOf;
...
Parameters = function(config = {}) {
  ...
  sanitize_commands = function(config) {
    ...
    for (name in ref) {
      ...
      results.push(sanitize_command(command));
    }
    return results;
  };
...
Parameters.prototype.parse = function(argv = process) {
  var _, config, index, name1, name2, option, params, parse, ref, ref1;
  ...
  params = {};
  parse = (config) => {
    var _, i, key, len, main, option, ref, ref1, ref2, shortcut, type, value, values;
    // Read options
    while (true) {
    ...
    if (argv.length !== index) {
      ...
      if (config.main) {
        ...
        if ((ref2 = config.commands) != null ? ref2[argv[index]] : void 0) {
  ...
  for (_ in ref1) {
    option = ref1[_];
    if (option.default != null) {
      if (params[name2 = option.name] == null) {
        params[name2] = option.default;
      }
    }
  }
  return params;
...
Parameters.prototype.stringify = function(params, options = {}) {
  var _, argv, config, key, keys, name1, name2, option, ref, ref1, stringify, value;
