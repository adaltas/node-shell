...
Parameters.prototype.parse = function(argv) {
  ...
  parse = function(config) {
    var key, main, option, options, shortcut, value, _i, _len, _ref, _ref1;
    while (true) {
    ...
    if (argv.length !== index) {
      ...
      if (config.main) {
        ...
        if ((_ref1 = config.actions) != null ? _ref1[argv[index]] : void 0) {
          config = _this.config.actions[argv[index]];
          params[_this.config.action] = argv[index++];
          parse(config, argv);
        } else {
  ...
  if (this.config.actions.length && argv[index].substr(0, 1) !== '-') {
    config = this.config.actions[argv[index]];
    if (!config) {
...
Parameters.prototype.stringify = function(script, params) {
  var actions, argv, config, index, stringify,
    _this = this;
  ...
  stringify = function(config) {
    var key, option, value, _ref, _ref1, _ref2;
    for (key in params) {
      value = params[key];
      if (key === _this.config.action || key === ((_ref = config.main) != null ? _ref.name : void 0)) {
        continue;
      }
      option = (_ref1 = config.options) != null ? _ref1[key] : void 0;
      if (option) {
        switch (option.type) {
          case 'boolean':
            if (value) {
        ...
        option = (_ref2 = _this.config.options) != null ? _ref2[key] : void 0;
        if (!option) {
          throw new Error("Invalid option '" + key + "'");
        }
        switch (option.type) {
  ...
  if (params[this.config.action]) {
    config = this.config.actions[params[this.config.action]];
    if (!config) {
