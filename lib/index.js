...
Parameters = function(config) {
  ...
  makeAction = function(action) {
    ...
    if ((_ref3 = action.strict) == null) {
      action.strict = config.strict;
    }
    action.shortcuts = {};
    if ((_ref4 = action.options) == null) {
      action.options = [];
    }
...
Parameters.prototype.parse = function(argv) {
  ...
  parse = function(config) {
    var key, main, option, options, shortcut, type, value, _i, _len, _ref, _ref1;
    while (true) {
...
Parameters.prototype.stringify = function(script, params) {
  ...
  if (params[this.config.action]) {
    ...
    keys[this.config.action] = params[this.config.action];
    stringify(config);
  }
  for (key in params) {
    value = params[key];
