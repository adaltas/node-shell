 ...
 */
var Parameters, load, pad, path, types,
  indexOf = [].indexOf;
 ...
 */
Parameters.prototype.run = function(argv = process) {
  var params, run;
  params = this.parse(argv);
  if (params.command) {
    run = this.config.commands[params.command].run;
  } else {
    run = this.config.run;
  }
  ...
  return run.call(this, params, argv, this.config);
};
