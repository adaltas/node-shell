...
Parameters = function(config = {}) {
  ...
  sanitize_command = function(command, parent) {
    ...
    if (command.flatten != null) {
      throw Error('Invalid Command: flatten cannot be declared inside a command');
    }
  ...
  if (config.flatten == null) {
    config.flatten = true;
  }
  if (typeof config.flatten !== 'boolean') {
    throw Error(`Invalid Configuration: flatten must be a boolean, got ${JSON.stringify(config.flatten)}`);
  }
  config.root = true;
...
Parameters.prototype.stringify = function(params, options = {}) {
  ...
  stringify = (config) => {
    ...
    // Recursive
    if (Object.keys(config.commands).length) {
      command = params[this.config.command];
      if (Array.isArray(command)) {
        command = params[this.config.command].shift();
      }
      argv.push(command);
