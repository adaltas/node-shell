...
Parameters = function(config) {
  var action, command, main, option, _i, _len, _ref, _ref1, _ref2;
  ...
  for (action in _ref) {
    ...
    this.requireds[action] = [];
    if (main) {
      this.bynames[action][main.name] = main;
    }
    if (main != null ? main.required : void 0) {
      this.requireds[action].push(main.name);
    }
    if (command.options) {
      _ref1 = command.options;
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        option = _ref1[_i];
        if ((_ref2 = option.type) == null) {
          option.type = 'string';
        }
        this.bynames[action][option.name] = option;
        this.shortcuts[action][option.shortcut] = option.name;
        if (option.required) {
          this.requireds[action].push(option.name);
        }
      }
    }
  }
...
Parameters.prototype.help = function(action) {
  var command, content, describeCommand, describeOption, synopsis, _ref, _ref1;
  command = this.config[action];
  ...
  if (action && action !== 'help') {
    ...
    synopsis += '[options...]';
    if (command.main) {
      synopsis += " [" + command.main.name + "]";
    }
    content = "NAME\n    startstop " + action + " - " + command.description + "\nSYNOPSIS\n    " + synopsis + "\nDESCRIPTION\n";
    return content += describeCommand(command);
  } else {
    content = "NAME\n    startstop - Control and monitor processes, restart on file changes\nSYNOPSIS\n    startstop action [options...]\n    where action is one of\n";
    _ref = this.config;
    for (action in _ref) {
      command = _ref[action];
      content += pad("      " + action, 24);
    ...
    _ref1 = this.config;
    for (action in _ref1) {
      command = _ref1[action];
      content += describeCommand(command);
    }
    content += "EXAMPLES\n    startstop help          Show this message\n    startstop help start    Describe the start action\n";
    return content;
  }
};
...
Parameters.prototype.decode = function(argv) {
  ...
  command = this.config[data.action];
  if (!command) {
