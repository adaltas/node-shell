...
Parameters = function(config) {
  var action, actions, makeAction, options, _i, _len, _ref;
  if (config == null) {
    config = {};
  }
  this.config = config;
    ...
    return function(config) {
      ...
      while (true) {
        ...
        if (config.strict && !option) {
          throw new Error("Invalid option '" + (shortcut || key) + "'");
        }
        if (!option) {
...
Parameters.prototype.help = function(action) {
  ...
  if ((action != null) && !command) {
    throw Error("Invalid command \"undefined\"");
  }
