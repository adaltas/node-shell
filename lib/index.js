...
Parameters = function(config) {
  ...
  options = function(command) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      results.push(command.shortcuts[option.shortcut] = option.name);
    }
    return results;
  };
  config.shortcuts = {};
...
Parameters.prototype.parse = function(argv) {
  ...
  parse = (function(_this) {
    return function(config) {
      var i, key, len, main, option, options, ref, ref1, ref2, shortcut, type;
      while (true) {
        ...
        switch (option.type) {
          ...
          case 'array':
            ...
            (ref1 = params[key]).push.apply(ref1, argv[index++].split(','));
        }
      }
...
Parameters.prototype.stringify = function(script, params) {
  ...
  stringify = (function(_this) {
    return function(config) {
      var i, key, len, option, ref, value;
      ref = config.options;
      for (i = 0, len = ref.length; i < len; i++) {
        option = ref[i];
        key = option.name;
