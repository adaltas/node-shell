...
var Parameters, array_to_object, clone, is_object, load, merge, pad, path, set_default, stream, types,
  indexOf = [].indexOf;
...
Parameters.prototype.configure = function(config = {}) {
  var collision, sanitize_command, sanitize_commands, sanitize_commands_enrich, sanitize_help, sanitize_main, sanitize_options, sanitize_options_enrich, sanitize_route;
  config = clone(config);
  this.config = config;
  // Sanitize options
  collision = {};
    ...
    for (name in ref) {
      ...
      if (!this.config.extended) {
        if (!config.root) {
          ...
          if (collide) {
            throw Error(['Invalid Option Configuration:', `option ${JSON.stringify(name)}`, `in command ${JSON.stringify(config.command.join(' '))}`, `collide with the one in ${(collision[name].length === 0 ? 'application' : JSON.stringify(collision[name].join(' ')))},`, "change its name or use the extended property"].join(' '));
          }
        }
      ...
      if (ref1 = option.type, indexOf.call(types, ref1) < 0) {
        throw Error(['Invalid Option Configuration:', `supported options types are ${JSON.stringify(types)},`, `got ${JSON.stringify(option.type)}`].join(' '));
      }
      if (option.shortcut) {
      ...
      if (option.one_of && !Array.isArray(option.one_of)) {
        throw Error(['Invalid Option Configuration:', 'option property "one_of" must be a string or an array,', `got ${option.one_of}`].join(' '));
      } else {
        results.push(void 0);
      }
    }
  ...
  sanitize_route = function(config) {
    ...
    if ((ref = typeof config.route) !== 'function' && ref !== 'string') {
      throw Error(['Invalid Route Configuration:', "accept string or function", !Array.isArray(config.command) ? "in application," : void 0, Array.isArray(config.command) ? `in command ${JSON.stringify(config.command.join(' '))},` : void 0, `got ${JSON.stringify(config.route)}`].filter(function(i) {
        return i;
      }).join(' '));
    }
  };
  sanitize_command = function(command, parent) {
    ...
    if (command.extended != null) {
      throw Error(['Invalid Command Configuration:', 'extended property cannot be declared inside a command'].join(' '));
    }
  ...
  sanitize_commands = function(config) {
    ...
    for (name in ref) {
      ...
      if (command.name && command.name !== name) {
        throw Error(['Incoherent Command Name:', `key ${JSON.stringify(name)} is not equal with name ${JSON.stringify(command.name)}`].join(' '));
      }
      command.name = name;
      if (command.command != null) {
        throw Error(['Invalid Command Configuration:', 'command property can only be declared at the application level,', `got command ${JSON.stringify(command.command)}`].join(' '));
      }
  ...
  if (typeof config.extended !== 'boolean') {
    throw Error(['Invalid Configuration:', 'extended must be a boolean,', `got ${JSON.stringify(config.extended)}`].join(' '));
  }
  config.root = true;
  ...
  sanitize_help = function(config) {
    ...
    if (typeof config.help.writer === 'string') {
      if ((ref = config.help.writer) !== 'stdout' && ref !== 'stderr') {
        throw Error(['Invalid Help Configuration:', 'accepted values are ["stdout", "stderr"] when writer is a string,', `got ${JSON.stringify(config.help.writer)}`].join(' '));
      }
    } else if (!(config.help.writer instanceof stream.Writable)) {
      if ((ref1 = config.help.writer) !== 'stdout' && ref1 !== 'stderr') {
        throw Error(["Invalid Help Configuration:", "writer must be a string or an instance of stream.Writer,", `got ${JSON.stringify(config.help.writer)}`].join(' '));
      }
    }
...
Parameters.prototype.route = function(argv = process, ...args) {
  var commands, config, configure, error, i, params, route, route_error;
  ...
  if (Array.isArray(argv)) {
    ...
    throw Error(['Invalid Arguments:', 'first argument must be an argv array or the process object,', `got ${JSON.stringify(argv)}`].join(' '));
  }
  route = (config, commands) => {
    ...
    if (!route) {
      ...
      if (!(Object.keys(config.commands).length || route)) {
        error = config.root ? Error(['Missing Application Route:', 'a \"route\" definition is required when no commands are defined'].join(' ')) : Error(['Missing Command Route:', `a "route" definition ${JSON.stringify(params[this.config.command])} is required when no child commands are defined`].join(' '));
      }
...
Parameters.prototype.parse = function(argv = process, options = {}) {
  ...
  parse = (config, command) => {
    var _, error, helping, j, key, leftover, len, main, option, params, ref, ref1, ref2, shortcut, type, value, values;
    ...
    while (true) {
      ...
      if (!shortcut && config.strict && !option) {
        error = Error(['Invalid Argument:', `the argument ${(shortcut ? "-" : "--")}${key} is not a valid option`].join(' '));
      ...
      switch (option.type) {
        ...
        case 'string':
          ...
          if (!((value != null) && value[0] !== '-')) {
            throw Error(['Invalid Option:', `no value found for option ${JSON.stringify(key)}`].join(' '));
          }
          params[key] = value;
          break;
        case 'integer':
    ...
    if (argv.length !== index) {
      ...
      if (config.main) {
        ...
        if (!config.commands[command]) {
          ...
          throw Error(['Invalid Argument:', `fail to interpret all arguments "${leftover.join(' ')}"`].join(' '));
        }
...
Parameters.prototype.helping = function(params, options = {}) {
  ...
  if (!options.extended) {
    if (!is_object(params)) {
      throw Error(["Invalid Arguments:", "`helping` expect a params object as first argument", "in flatten mode,", `got ${JSON.stringify(params)}`].join(' '));
    }
  } else {
    if (!(Array.isArray(params) && !params.some(function(cparams) {
      return !is_object(cparams);
    }))) {
      throw Error(["Invalid Arguments:", "`helping` expect a params array with literal objects as first argument", "in extended mode,", `got ${JSON.stringify(params)}`].join(' '));
    }
  }
  ...
  if (!options.extended) {
    if (params[this.config.command] && !Array.isArray(params[this.config.command])) {
      throw Error(['Invalid Arguments:', `parameter ${JSON.stringify(this.config.command)} must be an array in flatten mode,`, `got ${JSON.stringify(params[this.config.command])}`].join(' '));
    }
  ...
  search = function(config, commands, params) {
    ...
    if (helping) {
      if (options.extended && commands.length) {
        throw Error(['Invalid Argument:', '`help` must be associated with a leaf command'].join(' '));
      }
      return true;
    }
...
Parameters.prototype.help = function(commands = [], options = {}) {
  ...
  if (!Array.isArray(commands)) {
    throw Error(['Invalid Help Arguments:', 'expect commands to be an array as first argument,', `got ${JSON.stringify(commands)}`].join(' '));
  }
  config = this.config;
  configs = [config];
  for (i = j = 0, len = commands.length; j < len; i = ++j) {
    ...
    config = config.commands[command];
    if (!config) {
      throw Error(['Invalid Command:', `argument "${commands.slice(0, i + 1).join(' ')}" is not a valid command`].join(' '));
    }
    configs.push(config);
  }
  // Init
