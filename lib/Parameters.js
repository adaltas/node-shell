...
Parameters.prototype.configure = function(config = {}) {
  var sanitize_command, sanitize_commands, sanitize_commands_enrich, sanitize_help, sanitize_main, sanitize_options, sanitize_options_enrich;
  config = clone(config);
  this.config = config;
  // Sanitize options
  this.collision = {};
  sanitize_options = (config) => {
    var collide, name, option, ref, results;
    ...
    for (name in ref) {
      ...
      if (!this.config.extended) {
        if (!config.root) {
          ...
          collide = this.collision[name] && this.collision[name].filter(function(cmd, i) {
            return config.command[i] !== cmd;
          }).length === 0;
          if (collide) {
            throw error(['Invalid Option Configuration:', `option ${JSON.stringify(name)}`, `in command ${JSON.stringify(config.command.join(' '))}`, `collide with the one in ${(this.collision[name].length === 0 ? 'application' : JSON.stringify(this.collision[name].join(' ')))},`, "change its name or use the extended property"]);
          }
        }
        // Associate options with their declared command
        this.collision[name] = config.root ? [] : config.command;
      }
      // Normalize option
      results.push(option.name = name);
    }
    return results;
  };
  // Sanitize main
